...
% f. The values of x are normalized with \sqrt(0.5 𝚫x).
%
...
ii = 1;
if (opt)
	g = chebfun(@(y) (x(ii)+(y-t(ii))/(t(ii+1)-t(ii))*(x(ii+1)-x(ii)))/sqrt(0.5*(t(ii)+t(ii+1))-ft(1))-f(y), [ft(1), t(ii+1)],'splitting','on');
else
	g = chebfun(@(y) x(ii)/sqrt(0.5*(t(ii)+t(ii+1))-ft(1))-f(y), [ft(1), 0.5*(t(ii)+t(ii+1))],'splitting','on');
end
r = norm(g)^2;
...
if (opt)
  ii = n-1;
	g = chebfun(@(y) (x(ii) + (y-t(ii))/(t(ii+1)-t(ii))*(x(ii+1)-x(ii)))/sqrt(ft(2)-0.5*(t(ii)+t(ii+1)))-f(y), [t(ii), ft(2)],'splitting','on');	
else
  ...
  ii = n;
	g = chebfun(@(y) x(ii)/sqrt(ft(2)-0.5*(t(i-1)+t(ii)))-f(y), [0.5*(t(ii-1)+t(ii)), ft(2)],'splitting','on');
