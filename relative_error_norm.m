...
if (opt)
	...
	if (debug)
		h = chebfun(@(y) (x(1)+(y-t(1))/(t(2)-t(1))*(x(2)-x(1))), [ft(1), t(2)],'splitting','on');
		plot(g)
		plot(h)
	end
	...
	if (debug)
		h = chebfun(@(y) x(1), [ft(1), 0.5*(t(1)+t(2))],'splitting','on');
		plot(g)
		plot(h)
	end
end
...
for ii=2:n-2
	...
	if (opt)
		...
		if (debug)
			h = chebfun(@(y) (x(ii) + (y-t(ii))/(t(ii+1)-t(ii))*(x(ii+1)-x(ii))), [t(ii),t(ii+1)],'splitting','on');
			plot(g)
			plot(h)
		end
	else
		...
		if (debug)
			h = chebfun(@(y) x(ii), [0.5*(t(ii-1)+t(ii)), 0.5*(t(ii)+t(ii+1))],'splitting','on');
			plot(g)
			plot(h)
		end
	end
...
if (opt)
	...
	if (debug)
		h = chebfun(@(y) (x(n-1) + (y-t(n-1))/(t(n)-t(n-1))*(x(n)-x(n-1))), [t(n-1), ft(2)],'splitting','on');
		plot(g)
		plot(h)
	end	
	...
	if (debug)
		h = chebfun(@(y) x(n), [0.5*(t(n-1)+t(n)), ft(2)],'splitting','on');
		plot(g)
		plot(h)
	end
